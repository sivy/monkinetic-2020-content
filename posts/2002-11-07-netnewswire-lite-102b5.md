date: 2002-11-07 00:00
slug: netnewswire-lite-102b5
tags: ""
title: NetNewsWire Lite 1.0.2b5
---
Brent just keeps crankin' 'em out...
>  \[via [Ranchero.com](http://ranchero.com/)\] ...links in the HTML view are displayed on mouse-over; a display glitch with the Bandwidth Stats window was fixed; and old-but-still-current items are no longer erroneously marked as unread. (See the [change notes](http://ranchero.com/software/netnewswire/changenotes.php) for more info.) ([link](http://ranchero.com/software/netnewswire/beta.php))
